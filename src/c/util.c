...
error sanitize(struct wstr *const ws)
{
  ...
  for (wchar_t *s = ws->ptr, *end = ws->ptr + ws->len; s < end; ++s)
  {
    ...
    if (
        (0x00 <= c && c <= 0x1f) ||
        c == 0x22 || c == 0x2a || c == 0x2b || c == 0x2f ||
        c == 0x3a || c == 0x3c || c == 0x3e || c == 0x3f ||
        c == 0x7c || c == 0x7f)
    {
      *s = L'-';
    }
...
NODISCARD static error path_relative_path_to(struct wstr const *const path_from, DWORD const attr_from, struct wstr const *const path_to, DWORD const attr_to, struct wstr *const dest)
{
  struct wstr tmp = {0};
...
error is_same_file(struct wstr const *const file1, struct wstr const *const file2, bool *const same)
{
  ...
  *same = fi1.dwVolumeSerialNumber == fi2.dwVolumeSerialNumber && fi1.nFileIndexLow == fi2.nFileIndexLow && fi1.nFileIndexHigh == fi2.nFileIndexHigh;
  return eok();
}
...
error create_unique_temp_file(wchar_t const *const base_filename, wchar_t const *const ext, void *const data, size_t const datalen, struct wstr *const dest)
{
...
error create_unique_file(wchar_t const *const base_fullpath, wchar_t const *const ext, void *const data, size_t const datalen, struct wstr *const dest)
{
...
error from_mbcs(struct str const *const src, struct wstr *const dest)
{
  return from_cp(CP_ACP, src, dest);
}

error from_utf8(struct str const *const src, struct wstr *const dest)
{
  return from_cp(CP_UTF8, src, dest);
}

error to_mbcs(struct wstr const *const src, struct str *const dest)
{
  return to_cp(CP_ACP, src, dest);
}

error to_utf8(struct wstr const *const src, struct str *const dest)
{
  return to_cp(CP_UTF8, src, dest);
}
