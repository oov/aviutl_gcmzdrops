...
NODISCARD static error create_drop_data(HWND const window, POINT const pt, struct wstr const *const *const filepath, HDROP *const hdrop)
{
  if (!filepath)
  {
    return errg(err_invalid_arugment);
...
NODISCARD static error analyse_exedit_window_image(uint8_t const *const image, size_t const width, size_t const height, struct gcmz_analysed_info *const ai)
{
  if (!image)
  {
    return errg(err_invalid_arugment);
...
NODISCARD static error save_dib(uint8_t const *const image, size_t const width, size_t const height)
{
  ...
  wsprintfW(filename, L"gcmz-apierr-%04d%02d%02d-%02d%02d%02d.bmp", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
...
static INT_PTR CALLBACK input_dialog_wndproc(HWND const dlg, UINT const message, WPARAM const wparam, LPARAM const lparam)
{
  switch (message)
  {
  case WM_INITDIALOG:
