...
NODISCARD static error get_data(STGMEDIUM *const sm, IDataObject *const dataobj, CLIPFORMAT const format, LONG const index)
{
  if (!sm || !dataobj)
  {
...
NODISCARD static error read_custom_format(IDataObject *const dataobj, CLIPFORMAT const format, int const index, void **const p, size_t *const plen)
{
  if (!dataobj)
  {
    return errg(err_invalid_arugment);
...
NODISCARD static error files_add_temp_file(struct files *const files, wchar_t const *const base_filename, wchar_t const *const ext, wchar_t const *const mime, void *const data, size_t const datalen)
{
  struct wstr tmp = {0};
  bool created = false;
...
NODISCARD static error try_read_from_custom_format(IDataObject *const dataobj, WORD const format, wchar_t const *const mime, wchar_t const *const ext, struct files *const files)
{
...
static HRESULT STDMETHODCALLTYPE DragEnter(
    IDropTarget *This,
    ...
    DWORD *pdwEffect)
{
  ...
  {
    err = emsg(err_type_generic, err_abort, &native_unmanaged(NSTR("ドロップすべきファイルがないため処理を中断しました。")));
    goto cleanup;
  }
...
static HRESULT STDMETHODCALLTYPE DragOver(
    IDropTarget *This,
    DWORD grfKeyState,
    POINTL pt,
    DWORD *pdwEffect)
...
static HRESULT STDMETHODCALLTYPE DragLeave(
    IDropTarget *This)
{
  struct drop_target *const this = (void *)This;
  if (!this || this->super.lpVtbl != &g_drop_target_vtable)
...
static HRESULT STDMETHODCALLTYPE Drop(
    IDropTarget *This,
    IDataObject *pDataObj,
    DWORD grfKeyState,
    POINTL pt,
