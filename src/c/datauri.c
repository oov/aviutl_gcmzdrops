...
NODISCARD static error base64_decode(wchar_t const *const ws, size_t const wslen, void *const data, size_t const datalen)
{
  if (!ws)
  {
    return errg(err_invalid_arugment);
...
NODISCARD static error percent_decode(wchar_t const *const ws, size_t const wslen, void *const data, size_t const datalen)
{
  if (!ws)
  {
    return errg(err_invalid_arugment);
