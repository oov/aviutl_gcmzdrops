...
error lua_call_on_drop(struct lua *const l, struct files *const f, POINTL const point, DWORD const key_state, int const frame_advance)
{
  if (!l || !l->L || !f)
  {
    return errg(err_invalid_arugment);
...
error lua_call_on_drop_simulated(struct lua *const l, struct files *const f, POINTL const point, DWORD const key_state, int const frame_advance)
{
  if (!l || !l->L || !f)
  {
    return errg(err_invalid_arugment);
