...
static void friendly_report(error e, NATIVE_CHAR const *const s1, NATIVE_CHAR const *const s2, NATIVE_CHAR const *const s3)
{
  if (esucceeded(e))
  {
    return;
...
failed:
  ereport(sfree(&p));
  friendly_report(err, NSTR("一時ファイル \""), path->ptr, NSTR("\" を削除候補に登録できませんでした。ファイルは自動で削除されません。"));
}
